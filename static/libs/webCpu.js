...
WebAdapter.request = function (url, type, params, callback, dataType, options) {
  if (!url) {
    return false;
  }
  if (type !== "_jsonp") {
    var request = new WebRequest(url, type, dataType, options);
  ...
  if (this.httpObj) {
    ...
    return (function (query, callback) {
      if (type.toLocaleUpperCase() === "GET") {
        ...
        var data = WebTool.attachParams("", query).split("?")[1];
      }
...
ViewControl.prototype.updateView = function (task) {
  var t = task.container;
  if (t) {
    ...
    // else if(t.getElementsByClassName(this.config.name).length !==0) {
    // var temp = t.getElementsByClassName(this.config.name)[0];
    // temp.parentNode.removeChild(temp);
    // }
    // else {}
    if (!!task.data && task.data.constructor.name === "Array" && task.taskType === "multi") {
      for (var i = 0; i < task.data.length; i++) {
        var temp = this.initDom(task.data[i], t, task);
        if (typeof (this.callback) === "function") {
          this.callback(temp, task.data[i], task);
        }
      }
    } else {
      var temp = this.initDom(task.data, t, task);
      if (typeof (this.callback) === "function") {
  ...
  if (task.promise && typeof (task.promise.afterRenderCard) === "function") {
    task.promise.afterRenderCard(task.container, task.data, task);
...
var WebCpu = function (config, callback) {
  this.cards = {};
  if (config) {
    config.path = WebCpu.path;
    config.name = WebCpu.identity;
    this.initModule(config);
    var task = config;
    task.promise = {
...
WebCpu.prototype.changeStyle = function (name, fileName, path) {
  if (this[name] && this[name].css && this[name].css[0]) {}
}

WebCpu.prototype.update = function (name, task) {
  var task = task || {};
  try {
    task.container = document.querySelector("[module]") || document.body;
    //卸载原来的module样式
    this.uninstall("_main");
    this["_" + name].render(task);
    this.install("_" + name);
    this["_main"] = this["_" + name]
  } catch (e) {
...
WebCpu.prototype.initModule = function (config, path, name) {
  var _self = this;
  config.path = path || config.path;
  config.name = name || config.name;
  this["_" + config.name] = null;
  this.regComponent("_" + config.name, config, function (container, data, task) {
    _self.excuteTasks(document, {}, config.component);
    console.log("Execed successfully: tasks in the module");
  });
  //卸载默认加载的css
  this.uninstall("_" + config.name);
}
...
WebCpu.prototype.uninstall = function (name) {
  if (window["webCpu"] && webCpu[name] && webCpu[name].config) {
    if (typeof (webCpu[name].config.css) === "string" && webCpu[name].css && webCpu[name].css[0].parentNode) {
      webCpu[name].css[0].parentNode.removeChild(webCpu[name].css[0]);
    } else {
      for (var k in webCpu[name].config.css) {
        if (webCpu[name].css[k] && webCpu[name].css.parentNode) {
          webCpu[name].css[k].parentNode.removeChild(webCpu[name].css[k]);
        }
      }
    }
  }
}
WebCpu.prototype.install = function (name) {
  var _head = document.getElementsByTagName("head")[0];
  if (window["webCpu"] && this[name] && this[name].config) {
    if (typeof (this[name].config.css) === "string") {
      _head.appendChild(this[name].css[0]);
    } else {
      for (var k in this[name].config.css) {
        _head.appendChild(this[name].css[k]);
      }
    }
  }
}
