    ...
    if (data.header && data.header.length > 0 && data.records) {
      ...
      for (var i = 0; i < data.records.length; i++) {
        ...
        for (var j = 0; j < data.header.length; j++) {
          ...
          } else if(typeof (data.header[j].key) === "string") {
      ...
      if (data.page && data.page.total) {
        ...
        task.pagination = new Pagination($(tFoot).find("td")[0], data.page.total, data.page.size, data.page.current, function (n, size) {
