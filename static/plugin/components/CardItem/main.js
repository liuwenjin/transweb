(function () {
  ...
  webCpu["CardItem"].createMenuItem = function (menu) {
    var nav = $('<ul class="nav navbar-nav"></ul>');
    for (let k in menu) {
      var name = menu[k].name || menu[k];
      var item = $('<li ><a index=' + k + ' path="' + menu[k].path + '" href="#">' + name + '</a></li>');
      if (menu[k].children) {
        ...
        for (let i in menu[k].children) {
          ...
          var mItem = $('<li ><a index="' + i + '" path="' + tItem.path + '" href="#">' + tItem.name + '</a></li>');
    ...
    if (!data.title && !data.titleMenu && !data.titleMenu && !data.titleData && !data.breadcrumb) {
      ...
      if (data.titleMenu && data.titleMenu.constructor.name === "Array") {
        ...
        if (data.titleData.menu || data.titleData.leftMenu) {
          ...
          var nav = webCpu["CardItem"].createMenuItem(menu);
        ...
        if (data.titleData.rightMenu) {
          ...
          nav.find("li>a").on("click", function (e) {
            ...
            if (menu[index] && typeof (menu[index].callback) === "function") {
  ...
  webCpu["CardItem"].showTips = function (mCard, str, callback, options) {
    ...
    webCpu.CardItem.switchMask(card, "html",
      '<div style="text-align: center; " class="confirmDialog">\
            <div class="dialogTopArea" style="width: 100%; height: 40px; display: flex; align-items: center; display: flex;align-items: center;box-shadow: 0px -1px 0px inset #202020;"><ul class="nav nav-pills dealDialogArea"></ul>\
                <label class="dialogTitleArea" style="color: #ddd; position: absolute; left: 90px; margin: 0px; width: calc( 100% - 180px ); display: inline-block; padding: 2px; vertical-align: middle; text-align: center; "></label></div>\
            <button title="关闭" style="position: absolute;z-index: 9;right: 8px; top: 6px;" type="button" class="close" ><span style="font-size: 25px; color: #fff;" aria-hidden="true" class="glyphicon glyphicon-remove-circle"></span><span class="sr-only">Close</span></button>\
