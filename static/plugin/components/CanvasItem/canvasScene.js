...
canvasScene.prototype.active = function () {
    ...
    if (webCpu._animateRequest) {
        cancelAnimationFrame(webCpu._animateRequest);
    }
    ...
    function animate(callback) {
        ...
        webCpu._animateRequest = requestAnimationFrame(animate);
        return webCpu._animateRequest;
    }
    webCpu._animateRequest = animate();
    // this.resize(500, 500);
}
...
var RoleItem = function (scene, dna) {
    ...
    this.dna = dna;
    this.scene = scene;
    this.timeIndex = dna.timeIndex;
    this.context = scene.context;
    this.control = dna.info || {};
    this.control.count = 0;
    this.control.draw = dna.draw;
    info = dna.info || {};
    this.info = info;
...
RoleItem.prototype.adjustPos = function (control) {
    var pos = control.pos || this.dna.pos || [0, 0, 0];
    var offsetX = control.offsetX  || this.dna.offsetX || 0;
    var offsetY = control.offsetY || this.dna.offsetY || 0;

    if (pos[0] === "right") {
        this.x = this.context.canvas.width + offsetX;
    } else if (pos[0] === "middle") {
        this.x = this.context.canvas.width / 2 + offsetX;
    } else {}

    if (pos[1]=== "bottom") {
        this.y = this.context.canvas.height + offsetY;
    } else if (pos[1] === "middle") {
        this.y = this.context.canvas.height / 2 + offsetY;
    } else {}
}

RoleItem.prototype.getTimeIndex = function(gap) {
...
RoleItem.prototype.callback = function () {
    ...
    if (this.currentStage && this.currentStage.name && this.dna && this.dna.keyStages[this.currentStage.name]) {
        ...
        if(this.keyStages) {
            stages = this.keyStages[this.currentStage.name] || stages;
        }

        var index = 0;
        if(this.timeIndex && this.timeIndex[this.currentStage.name]) {
            var currentTime = (new Date()).getTime();
            var gap = currentTime - this.currentStage.startTime;
            if(this.dna && this.dna.clock && this.dna.clock.type === "audio" && this.dna.clock.audio) {
                gap = this.scene.audios[this.dna.clock.audio].currentTime * 1000;
            }

            var arr = this.timeIndex[this.currentStage.name];
            index = this.currentStage.index;
            if(typeof(index) === "number") {
                if(arr[index + 1] && arr[index + 1] < gap) {
                    index++;
                };
            }
            else {
            ...
            index = num % stages.length;
        }
        this.currentStage.index = index;
        this.control = stages[index];
        
    }
    var draw = this.control.draw;
    this.adjustPos(this.control);
...
RoleItem.prototype.__image = function (info) {
    ...
    if (!myShape) {
        this.sourceSize = this.info.sourceSize || [w, h];
...
RoleItem.prototype.__text = function (info) {
    var myInfo = info || this.info;
    this.dna.info = this.dna.info || {};
    var style = info.style || this.dna.info.style;
    myInfo.text = myInfo.text || this.dna.info.text;
    var text = myInfo.text || this.dna.info.text;
    this.updateStyle(style);
    if(text && text.constructor.name ===  "Array") {
        for(var i= 0; i < text.length; i++) {
            this.context.fillText(text[i], this.x, this.y + (myInfo.lineHeight || 40) * i);
        }
    }
    else {
