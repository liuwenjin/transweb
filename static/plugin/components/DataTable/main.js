    ...
    webCpu.regComponent("DataTable", config, function(container, data, task) {
        ...
        if (task.header && task.header.length > 0 && data) {
            ...
            if (task.numbered) {
                ...
                th.innerHTML = "<div style='display: flex; justify-content:center; align-items:center; width: 100%; height: 100%;' >" + (task.numbered.name || "序号") + "</div>";
                tr.appendChild(th);
            }
            ...
            for (var i = 0; i < task.header.length; i++) {
                ...
                var th = document.createElement("th");
                th.innerHTML = "<div style='display: flex; justify-content:center; align-items:center; width: 100%; height: 100%;'>" + task.header[i].name + "</div>";
                tr.appendChild(th);
            }
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < task.header.length; j++) {
                    ...
                    if (typeof(task.header[j].render) === "function") {
                        ...
                        td.innerHTML = task.header[j].render(v, data[i], task.header[j].key);
            ...
            if (task.option.page && task.option.page.total) {
                // var tFoot = table.createTFoot();
                // var tr = document.createElement("tr");
                // tr.innerHTML = "<td class='DataTable_page' colspan='" + count + "'></td>";
                // tFoot.appendChild(tr);
                task.pagination = new Pagination(task.footArea, task.option.page.total, task.option.page.size, task.option.page.current, function(n, size) {
